module github.com/converged-computing/fluence

go 1.26.0

require (
	github.com/converged-computing/qrmi-go v0.0.0-20260605012309-c5c8239ecbba
...
// Building against k8s.io/kubernetes as a library requires pinning every
// in-tree staging module to the SAME version as k8s.io/kubernetes. Keep these
// in lockstep with the k8s.io/kubernetes version above. Run `go mod tidy` after
// editing; you may need to add a few more of these as the dependency graph
// resolves (the full set mirrors the replace block in k8s.io/kubernetes/go.mod).
replace (
